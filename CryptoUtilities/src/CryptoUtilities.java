...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        NaturalNumber mod = nCopy.divide(two);
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (p.isZero()) {
            ...
            // {(n % m) * powerMod(n, p-1, m)} % m
            NaturalNumber pHalf = new NaturalNumber2(p);
            pHalf.divide(two);
            // store the original n to be multiplied at the end
            NaturalNumber realN = new NaturalNumber2(n);
            powerMod(n, pHalf, m);
            NaturalNumber nCopy = new NaturalNumber2(n);
            n.multiply(nCopy);
            n.transferFrom(n.divide(m));
            NaturalNumber initialNMod = new NaturalNumber2(realN.divide(m));
            // multiply by (n^p % m)
            initialNMod.multiply(n);
            n.transferFrom(initialNMod.divide(m));
        }
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 1; i < numCandidates; i++) {
                ...
                // if it's composite (is not prime), return false
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        // If n is a even, increase one to make it an odd number
        if (isEven(n)) {
            n.increment();
        }
        // Until next prime number generates, increase the odd number by two
        while (!isPrime2(n)) {
            n.increment();
            n.increment();
        }
