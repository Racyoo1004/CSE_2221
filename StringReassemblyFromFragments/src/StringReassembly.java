...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //print str1 without overlap part and print str2 to combine as a string
        String overlapStr = str1.substring(0, str1.length() - overlap) + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //setNoSub to remove any substring of str if it's added
        //setOrig to save original values of strSet
        Set<String> setNoSub = strSet.newInstance();
        Set<String> setOrig = strSet.newInstance();

        boolean contains = false;

        //Loop until strSet has any value with substring of str
        //If str is not a substring of any strSet values, check reversely
        while (!contains && strSet.size() > 0) {
            String piece = strSet.removeAny();
            if (piece.contains(str)) {
                contains = true;
            } else if (str.contains(piece)) {
                setNoSub.add(piece);
            }
            setOrig.add(piece);
        }
        ...
        //If str is not a substring of any strSet,
        //removed the reversely checked values and add str to strSet
        if (!contains) {
            strSet.remove(setNoSub);
            strSet.add(str);
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //Create a new set to save the input strings
        Set<String> lines = new Set1L<>();

        //Loop until reading the end of the input values
        while (!input.atEOS()) {
        ...
        //Create another set with the strings of lines
        Set<String> setStr = lines.newInstance();
        setStr.add(lines);

        //Loop to fill out the lines with filtering to avoid overlapping strings
        while (setStr.size() > 0) {
            String lineStr = setStr.removeAny();
            addToSetAvoidingSubstrings(lines, lineStr);
        }
        return lines;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        if (text.length() > 0) {
            //Find the index location of '~'
            int newLine = text.indexOf('~');

            //If ~ exist, print until ~ approaches and enter the recursion,
            if (newLine > -1) {
                String newText = text.substring(0, newLine);
                out.println(newText);
                String nextText = text.substring(newLine + 1, text.length());
                printWithLineSeparators(nextText, out);
                //if does not exist, just print the rest of the line
            } else if (newLine == -1) {
                out.println(text);
            }
        }
